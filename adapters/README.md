...
Adapters retain Remotion's default Chromium OpenGL behavior unless
`app.config.mjs` declares a validated `rendering.chromiumOpenGlRenderer`.
Renderer selection is per adapter because a backend that stabilizes one native
surface can destabilize another. Document any selection and rerun strict
delayed determinism and Webpack/Rspack verification across every surface. Do
not weaken pixel tolerances to mask renderer drift.
