...
| Fractionally transformed edges alternate by a one-channel value, and either bundler can produce either result | Raster backend drift              | Restore the shared `swangle` renderer and repeat the strict renders before blaming a bundler. Use a different backend only for a proven adapter requirement, set it after the shared config, and record the evidence. |
