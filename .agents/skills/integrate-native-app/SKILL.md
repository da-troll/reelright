...
- Retain Remotion's Chromium OpenGL default unless repeated evidence requires an
  adapter-local `rendering.chromiumOpenGlRenderer` setting. Document and
  reverify that setting under both strict pixel gates; never force one backend
  globally or add tolerance to conceal unexplained raster drift.
